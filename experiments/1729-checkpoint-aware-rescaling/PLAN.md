...
- Outer branch: `checkpoint-aware-inplace-rescaling`, created from `main` in
  the sibling worktree `flink-kubernetes-autoscaling-checkpoint`.
- Justin operator submodule: same branch name, based on pinned commit
  `70ab735` and published through the Andrewtangtang fork.
...
- Use checkpoint-specific image tags so shared `justin-modified` images are
  never overwritten.
...
3. Validate the Q20 manifest and operational scripts without changing the
   replay-based Q20 files. Confirm each rendered manifest contains one unique
   run-scoped checkpoint/savepoint/HA root and no unresolved placeholder.
4. Run matched Justin and DS2 Q20 pilots with a continuous producer and verify
