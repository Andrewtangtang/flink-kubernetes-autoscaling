# Q9 DS2 and Justin Re-evaluation

This experiment runs the state-heavy `q9_unique` Flink SQL query at the
...
The repository did not contain a Q9 deployment manifest. A temporary,
...
| `d1c51a4d3e...` | winning-bid Rank + sink | 1 | yes |
...
Using 18 for Q9 is therefore a reconstruction assumption, not a value stated
explicitly for Q9.
...
paper does not state Q9's source parallelisms, so auction P12 and bid P1 are
...
2026-07-24 showed only 18 GiB free and 166.7 GiB of reclaimable Docker build
...
```bash
kubectl kustomize experiments/1726-kafka-q9-unique/jobs/justin > /tmp/q9-justin.yaml
kubectl kustomize experiments/1726-kafka-q9-unique/jobs/ds2 > /tmp/q9-ds2.yaml

diff -u /tmp/q9-ds2.yaml /tmp/q9-justin.yaml
```
...
kubectl apply -k "experiments/1726-kafka-q9-unique/jobs/${POLICY}"
...
every subtask. Use `--task-regex 'Join|Rank'` to focus on Q9's
...
After the coordinator reports the tracked Q9 job, start the initial producer
