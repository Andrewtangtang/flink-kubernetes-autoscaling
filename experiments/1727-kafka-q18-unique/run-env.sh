...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  echo "Source this file so the Q18 run configuration remains in the current shell:" >&2
  echo "  source experiments/1727-kafka-q18-unique/run-env.sh" >&2
  exit 1
fi
...
printf '%s\n' \
  "Loaded Q18 re-evaluation configuration:" \
