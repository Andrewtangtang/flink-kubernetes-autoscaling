...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  echo "Source this file so the Q19 run configuration remains in the current shell:" >&2
  echo "  source experiments/1728-kafka-q19-unique/run-env.sh" >&2
  exit 1
fi
...
printf '%s\n' \
  "Loaded Q19 re-evaluation configuration:" \
