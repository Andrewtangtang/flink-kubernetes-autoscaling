...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  echo "Source this file so the Q4 run configuration remains in the current shell:" >&2
  echo "  source experiments/1725-kafka-q4-unique/run-env.sh" >&2
  exit 1
fi
...
printf '%s\n' \
  "Loaded Q4 re-evaluation configuration:" \
